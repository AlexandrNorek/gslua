...
local check = ui.new_multiselect("lua", "a", "Check for cyrillic: ", { "Chat", "Names" })
...
client.set_event_callback("round_poststart", function()
    if not multifix(ui.get(check), "Names") then return end
    local players = entity.get_all("CCSPlayer")
    for i = 1, #players do
        local name = entity.get_player_name(players[i])
        if string.find(name, alphabet[i]) then
            local xuid = GameStateAPI.GetPlayerXuidStringFromEntIndex(players[i])
            reportenemy(xuid, name)
        end
    end
end)
client.set_event_callback("player_chat", function(e)
    if not multifix(ui.get(check), "Chat") then return end
